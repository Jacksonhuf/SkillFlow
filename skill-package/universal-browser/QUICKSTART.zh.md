...
需要电脑上装有 **Google Chrome** 和 **Python 3.12+**（没有 Python 时启动器会带你去下载页，安装时勾选 *Add python.exe to PATH*）。
...
会自动弹出一个网页（只在你本机可访问）。第一次打开时系统自动准备浏览器驱动，页面顶部会显示「环境就绪」；如果缺什么，会给出 **「一键修复」** 按钮，点它即可。
...
| 现象 | 处理 |
|------|------|
| 双击后提示没有 Python | 到 https://www.python.org/downloads/ 安装，勾选 *Add python.exe to PATH*，再双击 |
