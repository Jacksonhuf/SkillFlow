...
- **标准小包**（`universal-browser-0.1.0.zip`）：只有 `SKILL.md` + `references/`，适合 Hub 只分发指令、运行时由平台统一部署。
- **完整包**（`universal-browser-full-0.1.0.zip`）：本说明 + `templates/` + `runtime/`，适合 Hub **一个 ZIP 搞定** 指令与执行层。
