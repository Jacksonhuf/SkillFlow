...
- ✅ 引擎自动选择 `browser/auto_engine.py`（`--engine auto` 为默认）：**Playwright 为绝对默认**，驱动本机 Chrome；缺 `playwright` 包时静默 `pip install`（`UNIVERSAL_BROWSER_NO_AUTO_PIP=1` 关闭）；仅当 Playwright 不可用才回退 chrome-use（已装 CLI → Windows 引导下载）
